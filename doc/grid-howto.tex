...
\includegraphics[width=0.32\textwidth]{EPS/adaptiveintegration_alberta2d.eps}\hfill
...
the follwing principles: i) Separation of data structures and
...
You have checked out two directories \lstinline!dune! and 
...
are used and the overlap is set to one element. Periodicity is not not
used.  
...
configured with a dimension (\lstinline!--with-problem-dim=2!,
\lstinline!--with-world-dim=2!) and only this dimension can then be
used. The dimension from the configure run is available in the macro
\lstinline!DUNE_PROBLEM_DIM! in the file \lstinline!config.h! (see
...
The next listing shows the instantiation of \lstinline!ALU3dGrid!
objects. ALU3d supports either tetrahedral or hexahedral grids and the
element type has to be choosen at compile-time. Therefore, the element
type is a template parameter of the grid class. The
\lstinline!variant! parameter must be 1. 
...
The class template
\lstinline!Dune::LeafMultipleCodimMultipleGeomTypeMapper! 
provides an index-based mapper where the entities in the subset
$E^\prime$ are all leaf entities and can further be selected depending
on the codimension and the geometry type. To that end the second
template argument has to be a class template with one integer template
parameter containing a method \lstinline!contains!. Just look at the
example \lstinline!P0Layout!. When the method \lstinline!contains!
returns true for a combination of dimension, codimension and
geometry type then all leaf entities with that dimension, codimension and
geometry type will be in the subset $E^\prime$. The mapper object is
constructed in lines 31/32. A similar mapper is available also for the
entities of a grid level. 

The data vector is allocated in line 35. Here we use a
...
next three statements within the loop body compute the position of the
center of the element in global coordinates. Then the essential
statement is in line 52 where the function is evaluated and the value
is assigned to the corresponding entry in the \lstinline!c! array. The
evaluation of the map $m$ is performed by \lstinline!mapper.map(*it)!
where \lstinline!*it! is the entity which is passed as a const reference to
the mapper.

The remaining lines of code produce graphical output. Lines 56-59
produce an output file for the Visualization Toolkit (VTK), \cite{VTK}, in its XML
format. If the grid is distributed over several processes the
...
visualize these files. Lines 62-65 enable online interactive
visualization with the Grape, \cite{Grape}, graphics package, if it is
...
\item Pass 1 instead 0 as the second last argument of
  \lstinline!grape.displayVector!. This argument is the polynomial
...
\begin{equation}
C_i^{n+1} = C_i^n - \Delta t^n \sum\limits_{\gamma_{ij}}
  \underbrace{\frac{|\gamma_{ij}|}{|\omega_i|}
...
\includegraphics[width=0.48\textwidth]{EPS/intersection.eps}\hfill
\includegraphics[width=0.48\textwidth]{EPS/islocalref.eps}
\caption{Left: inntersection with other elements and the boundary,
  right: intersections in the case of locally refined grids.}
\label{Fig:IsIt}
\end{figure}

Lines 30-128 contain the loop over all leaf elements where the optimum
$\Delta t^n$ and the cell updates $\delta_i$ are computed. The update
vector is allocated in line 23, where we assume that \lstinline!V! is
a container with copy constructor and size method. 

The computation of the fluxes is done in lines 55-123. An
\lstinline!IntersectionIterator! is used to access all intersections
$\gamma_{ij}$ of a leaf element $\omega_i$. For a full documentation
of the \lstinline!IntersectionIterator! we refer to
\begin{center}
\href{http://hal.iwr.uni-heidelberg.de/dune/doc/doxygen/html/classDune_1_1IntersectionIterator.html}%
{{\small\texttt{http://hal.iwr.uni-heidelberg.de/dune/doc/doxygen/html/classDune\_1\_1IntersectionIterator.html}}}
\end{center}
An Intersections is with
another element $\omega_j$ if the \lstinline!neighbor()! method of
the iterator returns true (line 83) or with the external boundary if
\lstinline!boundary()! returns true (line 118), see also left part of Figure
\ref{Fig:IsIt}. An intersection $\gamma_{ij}$ is
described by several mappings: (i) from a reference element of the
intersection (with a dimension equal to the grid's dimension minus 1) to the
reference elements of the two elements $\omega_i$ and $\omega_j$ and
(ii) from a reference element of the intersection to the global
coordinate system (with the world dimension). If an intersection is
with another element then the \lstinline!outside()! method returns an
\lstinline!EntityPointer! to an entity of codimension 0.
...
The $\Delta t^n$ calculation is done in line 126 where the minimum
over all cells is taken. Then, line 131 multiplies the optimum $\Delta
t^n$ with a safety factor to avoid any instability due to round-off
errors. 

Finally, line 134 computes the new concentration by adding the scaled
update to the current concentration.
...
The function \lstinline!timeloop! constructs a mapper an allocates the
concentration vector with one entry per element in the leaf grid. In
line 39 this vector is initialized with the initial concentration and
the loop in line 44-50 evolves the concentration in time. Finally, the
simulation result is written to a file in line 53.
...
The loop in lines 42-72 computes the indicator values $\eta_i$ as well
as the global minimum and maximum $\overline{C},\underline{C}$. Then
the next loop in lines 76-94 marks the elements for refinement. Lines
97-124 construct a map that stores for each element in the mesh (on
all levels) the average of the element values in the leaf elements of the
subtree of the given element. This is accomplished by descending from
the fine grid levels to the coarse grid levels and thereby adding the
value in an element to the father element. The jey into the map is the
global id of an element. Thus the value is accessible also after mesh
modification.

Now grid can really be modified in line 128 by calling the
\lstinline!adapt()! method on the grid object. The mapper is updated
to reflect the changes in the grid in line 129 and the concentration
vector is resized to the new size in line 130. Then the values have to
be interpolated to the new elements in the mesh using the map and finally to be
transferred to the resized concentration vector. This is done in the
loop in lines 133-173.
...
\section{Dune Data Decomposition Model}
...
\begin{center}
\includegraphics[width=\textwidth]{./EPS/partitionsingle.eps}
\end{center}
...
The first difference to the sequential version is in line 7 where it
is checked that the grid provides an overlap of at least one
element. The overlap my be either of partition type \textit{overlap}
or \textit{ghost}. The finite volume scheme itself only computes the
updates for the elements with partition type \textit{interior}.
...
\lstinline!PartitionIteratorType! as shown in lines
17-18. If the argument \lstinline!All_Partition! is given then all
entities are processed, regardless of their partition type. This is
also the default behavior of the level and leaf iterators. If the
partition iterator type is specified explicitely in an iterator the
same argument has also to be specified in the begin and end methods on
the grid as shown in lines 35-36.

The next change is in line 131 where the computation of the optimum
stable time step is restricted to elements of partition type
\textit{interior} because only thos elements have all neighboring
elements locally available. Next, the global minimum of the time steps
sizes determined in each process is taken in line 137. For collective
communication each grid returns a collective communication object with
its \lstinline!comm()! method which allows to compute global minima
and maxima, sums, broadcasts and other functions.
...
the other processes. This is done in lines 141-144 using the data
handle described above. The \lstinline!communicate! method on the grid
uses the data handle to assemble the message buffers, exchanges the
data and writes the data into the user's data structures.
...
The only essential difference to the sequential program is in line 51
where the printing of the data of the current time step is restricted
